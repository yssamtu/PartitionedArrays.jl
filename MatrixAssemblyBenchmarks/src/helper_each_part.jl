function get_execution_time_each_part(path)
...
function get_execution_time_each_part(path, buildmat, rebuildmat)
    function get_maximum(v)
        np = length(v)
        nruns = length(v[1])
        # while np > 1
...
function get_all_execution_times_each_part(params)
    ...
    for method in methods
        ...
        execution_times[method] = get_execution_time_each_part(path)
    end
    execution_times
end
